...
export const handleSqlQuery = async (req: AuthenticatedRequest, res: Response) => {
  ...
  try {
    ...
    const metadata = buildAIQueryMetadata(rbacUserRole, req.user?.id, question);
    const answer = await queryHospitalDatabase(question, aiUserRole, metadata);
    return res.json({ answer });
  } catch (error) {
    console.error('AI SQL query error:', error);
    return res.status(500).json({ message: 'Unable to generate answer from the hospital SQL chain' });
  }
};
...
export const handleAgentQuery = async (req: AuditedRequest, res: Response) => {
  ...
  try {
    ...
    return res.status(500).json({ message: 'Unable to generate answer from the hospital AI agent' });
  }
};
...
export const handleChatQuery = async (req: AuditedRequest, res: Response) => {
  ...
  try {
    ...
    return res.status(500).json({ message: 'Unable to generate answer from the hospital AI chat service' });
  }
};
...
export const handleTTS = async (req: AuthenticatedRequest, res: Response) => {
  ...
  try {
    ...
    if (auditLog.wasFiltered) {
      console.log(
        `[VoicePrivacy] Sensitive content filtered for user ${req.user?.id || 'unknown'}. Keywords: ${auditLog.sensitiveKeywordsDetected?.join(', ')}`,
      );
    }
